...
if [[ <CREATE REVOKE FUNCTION> == 'true' ]]; then
    ...
    secret_arn=$(aws secretsmanager describe-secret --secret-id <DEWPOINT JOB ID>-secret-runtime | jq -r .ARN)

    declare -A revoke_lambda_function
    revoke_lambda_function[Configuration.Environment.Variables.BIGIQ_ADDRESS]="<BIGIQ ADDRESS>"
    revoke_lambda_function[Configuration.Environment.Variables.BIGIQ_USERNAME]="admin"
    revoke_lambda_function[Configuration.Environment.Variables.BIGIQ_SECRET_ARN]=$secret_arn
    revoke_lambda_function[Configuration.Environment.Variables.BIGIQ_LICENSE_POOL]="<BIGIQ LICENSE POOL>"
    revoke_lambda_function[Configuration.Environment.Variables.BIGIQ_UTILITY_SKU]="<BIGIQ UTILITY SKU>"
    revoke_lambda_function[Configuration.Environment.Variables.TENANT]="<TENANT>"
...
response=''
if [[ <CREATE REVOKE FUNCTION> == 'true' ]]; then
response=$(verify_lambda_function "revoke_lambda_function" "LambdaBigIqRevoke")${spacer}
fi
if [[ <CREATE AMI LOOKUP FUNCTION> == 'true' ]]; then
response=$(verify_lambda_function "ami_lambda_function" "AMIInfoFunction")${spacer}
fi

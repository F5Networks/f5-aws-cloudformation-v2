...
if [[ "<CREATE AUTOSCALE GROUP>" == "true" ]]; then
    parameters="ParameterKey=mgmtSubnet,ParameterValue=\"$subnetAz1\" "
    parameters+="ParameterKey=mgmtSubnets,ParameterValue=\"$subnetAz1,$subnetAz2\" "
else
   parameters="ParameterKey=mgmtSubnet,ParameterValue=\"$subnetAz1\" "
   parameters+="ParameterKey=mgmtSubnets,ParameterValue=\"$subnetAz1\" "
fi

parameters+="ParameterKey=bastionSecurityGroupId,ParameterValue=$bastionSecurityGroupId \

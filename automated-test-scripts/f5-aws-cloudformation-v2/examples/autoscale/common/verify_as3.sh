...
if [[ "<PROVISION EXAMPLE APP>" == "false" ]]; then
    ...
    if [[ "<PROVISION PUBLIC IP>" == "false" ]]; then
        ...
        AS3_RESPONSE=$(ssh -o "StrictHostKeyChecking=no" -o ConnectTimeout=7 -i ${bastion_private_key} ec2-user@"$bastion_ip" "curl -skvvu 'admin:${PASSWORD}' https://${bigip_private_ip}:${MGMT_PORT}/mgmt/shared/appsvcs/declare" | jq -r .)
    else

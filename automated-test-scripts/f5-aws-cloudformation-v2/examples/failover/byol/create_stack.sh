...
if [[ "<RUNTIME INIT CONFIG 01>" == *{* ]]; then
    ...
    if [[ "<PROVISION EXAMPLE APP>" == "false" ]]; then
        declare -a runtime_init_config_files=(/$PWD/examples/failover/bigip-configurations/runtime-init-conf-3nic-byol_instance01.yaml /$PWD/examples/failover/bigip-configurations/runtime-init-conf-3nic-byol_instance02.yaml)
    else
        declare -a runtime_init_config_files=(/$PWD/examples/failover/bigip-configurations/runtime-init-conf-3nic-byol_instance01_with_app.yaml /$PWD/examples/failover/bigip-configurations/runtime-init-conf-3nic-byol_instance02_with_app.yaml)
    fi
    counter=1

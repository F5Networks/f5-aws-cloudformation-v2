...
if [[ "<PROVISION PUBLIC IP>" == "false" ]]; then
    ...
    SSH_RESPONSE=$(ssh -o "StrictHostKeyChecking no" -i /etc/ssl/private/dewpt_private.pem -o ProxyCommand="ssh -o 'StrictHostKeyChecking no' -i /etc/ssl/private/dewpt_private.pem -W %h:%p ubuntu@$bastion_ip" admin@"$bigip_private_ip" 'list auth user admin')

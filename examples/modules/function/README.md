...
- When creating BIG-IQ revoke function, you must supply the ARN of the AWS Secrets Manager secret containing the password of the BIG-IQ device used to license the BIG-IPs. The secret must be stored as a string; an example AWS CLI command to create this secret: ```aws secretsmanager create-secret --name mySecretName --secret-string '<BIGIQ PASSWORD>' --region us-east-1```
...
## Important Configuration Notes
 ...
 - If the BIG-IQ device where BIG-IPs are licenses uses a public IP address, leave the values for the bigIqSubnetId and bigIqSecurityGroupId parameters empty.
 
 - BIG-IQ license class must use unreachable: Because the Lambda function for license revocation filters license assignments based on the assignment tenant value, you must specify "reachable: false" in your F5 Declarative Onboarding license class declaration. See the F5 Declarative Onboarding [documentation](https://clouddocs.f5.com/products/extensions/f5-declarative-onboarding/latest/big-iq-licensing.html#license-class) for more information.
...
| bigIqAddressType | No | Specify the address type of the BIG-IQ device. [public, private] Required when createRevokeFunction set to yes. |
| bigIqLicensePool | No | The name of the BIG-IQ license pool used during BIG-IP licensing via BIG-IQ. Required when createRevokeFunction set to yes. |
...
| createAmiLookupFunction | No | Choose true to creates ami lookup serverless function. |
| createRevokeFunction | No | Choose true to creates bigiq revoke license serverless function. |
...
You can request ami id for a specified bigip once ami lookup function has been deployed. The function requires service token for the function created by function temple, the region a specified bigip is to be deployed, the string to use for filter, and owner id. In the following template snip-it, the service token value is found using outputs section of the deployed function template, region value is resolved using intrinsic template function, OSName value is set by parameter named 'imageName', and OwnerId is statically set using aws-marketplace alias id.
```yaml
  AmiInfo:
    Type: 'Custom::AMIInfo'
    Properties:
...
The ami lookup function is setup to filter down a list of images that match the given string within the description field of each image. A single ami id is returned using the date stamp to determine latest image.
You can determine what will be returned using aws cli. For example:
